...
- ⚠️ **`--text-sec` sin redirigir = texto casi invisible (sep 2026).** Cada tema redefine un bloque de variables CSS globales (`--bg`, `--text`, `--muted`, `--purple*`, `--dark`…) para verse legible sobre su fondo oscuro propio. `--text-sec` (texto de párrafo dentro de `callout` y otras secciones de `lesson.jsx`) se quedó SIN redirigir en `lab` y `time-travel` (`escape-room` tampoco la tenía hasta este fix; `detective` sí la traía bien) — como los temas activan `applyLightOnly()` (app.jsx), esa variable seguía con su valor de modo claro (`#4A4A5E`, gris oscuro para fondo blanco) sobre un fondo casi negro: prácticamente ilegible. Al agregar un tema nuevo o tocar estos bloques, **verificar que TODAS las variables de texto de `:root` tengan su redirección** (no solo `--text`/`--dark`/`--muted`) — es fácil que un texto secundario se cuele sin dueño.
...
- **Flujo por pregunta (lo dicta el profe):** `lobby → question → reveal → explanation → (siguiente pregunta)`, repetido pregunta por pregunta, y `podium` al terminar la última. El estado vive en `live_sessions`; estudiantes y host se suscriben por realtime (`subscribeSession`/`subscribeParticipants` en `lib/liveClient.js`) + red de seguridad por poll cada 7s y en `visibilitychange`. ⚠️ **Sin tabla de posiciones intermedia (sep 2026):** existió una fase `leaderboard` entre `explanation` y la siguiente pregunta, pero se quitó a pedido — el ranking parcial competía por atención con la explicación de cada pregunta. El ranking ahora vive solo en el podio final. La fase sigue listada en el comentario del esquema de `0022_live_classroom.sql` (migración ya aplicada, no se edita en retrospectiva) pero el frontend ya no la usa.
...
- **Ruta bloqueada mientras hay clase pendiente de unión (ago 2026).** Antes, mientras el profesor tenía una sesión activa y el estudiante no se había unido, el `GuidedSessionBanner` era solo un aviso: el mapa/lección/reto de abajo seguían siendo 100% interactivos (y el aviso se podía descartar con ✕, quedando sin rastro). Ahora `app.jsx` calcula `pendingGuided = role==='student' && !!guided.session && !isClonePage` (cierto en cuanto el profe crea la sesión — antes incluso de que arranque la primera pregunta — y falso de nuevo apenas la sesión termina o el estudiante se une) y con eso desenfoca + `pointer-events:none` el contenido de `<main>`, agregando encima `RouteLockOverlay` (`GuidedSessionBanner.jsx`) con un botón "Unirme a la clase". La ruta se sigue viendo (de fondo, borrosa) pero no se puede tocar hasta unirse — el banner superior sigue existiendo tal cual (dismissible, cosmético) pero ya no es lo único que impedía nada. Se exceptúan las páginas del piloto clon (`CLONE_PAGES`): viven al lado de la ruta de formación, no dentro de ella (§13), así que no se bloquean.
- **Candado por-curso "requires_live_to_start" (sep 2026).** Distinto del bloqueo anterior (que es temporal, solo mientras hay una sesión activa esperando unión): este es un candado permanente hasta la PRIMERA vez que el estudiante participa en una Clase en Vivo de ese curso — pensado para un curso que se recorre en clase (no de forma autodirigida) hasta que arranca. Es opt-in por curso (columna `courses.requires_live_to_start`, `0063`), hoy solo activo en "Sala de Escape - Matemáticas" (curso base y su fork): el resto de la plataforma no se ve afectado. `selectRequiresLiveToStart` (store.jsx) resuelve sobre `effectiveCourseId` (el fork si aplica, igual que el resto del sistema de fork). `app.jsx` consulta `hasCompletedLiveSession(courseId)` → RPC `has_completed_live_session` (SECURITY DEFINER, porque `live_participants.user_id` no es legible por la tabla directo, 0029) que revisa si el estudiante (`auth.uid()`) tiene una fila en `live_participants` de una sesión de ese curso con `status='ended'`. Mientras da `false`, `waitingFirstLive` desenfoca `<main>` igual que `pendingGuided` y pinta `WaitingFirstLiveOverlay` (sin botón — puede que hoy no haya ninguna sesión abierta, a diferencia de `RouteLockOverlay`). Si ambos candados aplicaran a la vez, gana `pendingGuided` (hay algo concreto que hacer: unirse). Una vez el estudiante termina su primera clase en vivo, `hasLiveHistory` pasa a `true` para siempre — no hay "re-bloqueo" entre clases, es deliberado.
  - ⚠️ **Un fork NO hereda `requires_live_to_start` del curso padre automáticamente** — hay que activarlo en ambos (o en el fork puntual que se vaya a usar) a mano, como cualquier otra columna de `courses` al forkear.
- ⚠️ **Crash al entrar a una clase en vivo si un canal viejo no se limpió a tiempo (sep 2026).** `subscribeSession`/`subscribeParticipants`/`subscribeCourseSessions` (`liveClient.js`) creaban el canal con `supabase.channel(nombre)` directo. Si un mount anterior (otra sesión en vivo, o una navegación rápida entre cursos) dejó un canal con ESE MISMO nombre todavía registrado en el cliente sin que `removeChannel` alcanzara a limpiarlo, `supabase.channel(mismoNombre)` podía devolver una instancia que el cliente ya trataba como suscrita, y el `.on('postgres_changes', ...)` de la nueva suscripción reventaba con `cannot add postgres_changes callbacks... after subscribe()` — el `ErrorBoundary` atrapaba el crash y el estudiante veía "Algo salió mal" y volvía a la ruta anterior al recargar. Arreglado con `freshChannel(nombre)`: antes de crear el canal, remueve cualquier canal existente con ese mismo topic (`supabase.getChannels()` filtrado por `.topic === 'realtime:'+nombre`). Si se agrega un canal nuevo en este archivo, usar `freshChannel` en vez de `supabase.channel` directo.
- ⚠️ **El estudiante se quedaba en pantalla blanca al terminar la clase (sep 2026).** `useGuidedSession` (`liveClient.js`) usaba `fetchActiveSessionForCourse` (que excluye a propósito `status='ended'`, es la consulta pensada para el banner pre-unión) también para seguir la sesión **ya unida**. Apenas el profesor finalizaba, `guided.session` pasaba a `null` en vez de `{status:'ended'}`: `GuidedClassView` (`if (!session || !participant) return null`) dejaba de pintar nada, y el `useEffect` de `app.jsx` que agenda `guided.leave()` a los 5s (para devolver al estudiante al mapa) nunca se disparaba porque esperaba `session.status==='ended'`, que jamás llegaba. Arreglado separando el hook en dos efectos: mientras NO está unido sigue usando la consulta por curso (excluye `ended`, correcta para el banner); apenas hay `participant`, cambia a seguir esa sesión puntual por id con `fetchSession`/`subscribeSession` (sin filtro de estado) — así `'ended'` sí llega y el flujo de vuelta al mapa funciona. De paso deja de correr un segundo poll de 7s redundante (course-scoped) mientras la clase está en curso.
- **Informe de cierre (ago 2026).** Tras finalizar la clase (podio), el profesor puede abrir "📄 Generar informe de cierre" (`ClosingReport` en `LiveHost.jsx`): un documento con el ranking final (misma fuente que el podio, `fetchParticipants`) y un cuadro de **comentarios generales de la sesión** que edita y guarda con `saveLiveClosingNotes` (`liveClient.js`) → columna `live_sessions.closing_notes` (`0061_live_session_closing_notes.sql`). ⚠️ Sin RPC nueva: el host ya puede escribir directo su propia fila de `live_sessions` (policy `ls_host_all` de 0022, `for all using/with check host_id = auth.uid()`), así que el guardado es un `update` normal. Se imprime con `window.print()` + `@media print` scoped a `#live-report-print`, mismo patrón que el acta de cierre (§12) y la tabla de efectividad (§13) — pero **sin** depender de `cloneShared.jsx`: ese archivo es del piloto clon temporal (pensado para borrarse de una pieza), así que el informe de cierre trae su propio CSS de impresión inline en `LiveHost.jsx` en vez de importar de ahí. El informe se genera **en el momento** (mientras la sesión sigue en memoria en `Control`); no hay pantalla para revisitar sesiones pasadas — si el profesor sale del panel sin generarlo, se pierde.
...
6. **Migraciones sin aplicar:** `0058` (bucket `corpus-normativo`), `0059` (regla de conteos en `clone_effectiveness`), `0061` (`live_sessions.closing_notes`, informe de cierre de la clase en vivo, §8), `0063` (`courses.requires_live_to_start` + RPC `has_completed_live_session`, candado de ruta hasta la primera Clase en Vivo — **deshabilitado a propósito** en Matemáticas por ahora, ver `scripts/deshabilitar_candado_matematicas.sql`), `0065`/`0066`/`0067` (mismo patrón de `0062`+`0064` replicado en los forks "— mi versión" de Detectives/Lenguaje, Viajeros del Tiempo/Ciencias Sociales y Laboratorio/Ciencias Naturales: 2 preguntas en vivo intercaladas + contenido real en los 5 "Pendiente" de cada una) están escritas, pero **no se han corrido en el SQL Editor**. `0062` y `0064` (Matemáticas) ya se aplicaron.
