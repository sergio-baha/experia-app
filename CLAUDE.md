...
- **Guion estándar de sesión (ago 2026, `0061`, sin correr).** Las 4 rutas temáticas (Laboratorio, Detectives, Sala de Escape, Viajeros del Tiempo) están pensadas para dictarse como una sesión presencial de 2h10 con la misma estructura pedagógica en las 4: rompehielo → encuadre de la Prueba Saber → diagnóstico con resultados previos → práctica nivel medio-alto → activación física → práctica nivel alto → cierre reflexivo. `0061_seed_live_rundown.sql` **reemplaza** toda la ruta (`course_modules`) del curso BASE de cada tema (nunca los forks de colegio) por 7 módulos tipo lección con esos tiempos, más el título y la frase de apertura en la voz del tutor de esa ruta (mismo personaje que `characters.jsx`). Quedan como módulos normales, totalmente editables desde el Editor de Ruta — no hay ningún concepto de "guion" en el código de la app, es contenido sembrado como cualquier otro (§6). Los bloques de práctica y el de encuadre traen una nota "🚧 Pendiente" en su contenido señalando qué banco (preguntas / actividades / documento) falta cargar; el tutor edita el módulo y lo completa ahí cuando ese material esté listo. ⚠️ Los forks por colegio que ya existan de estos cursos NO se actualizan solos — el tutor de ese colegio usa "Importar ruta de otro colegio" después de correr la migración.
...
6. **Migraciones sin aplicar:** `0058` (bucket `corpus-normativo`), `0059` (regla de conteos en `clone_effectiveness`) y `0061` (reemplaza la ruta de los 4 cursos temáticos por el guion estándar de Aula en Vivo — ver §8) están escritas y probadas, pero **no se han corrido en el SQL Editor**
